 ...
 * {@link DatabaseConfig#PROPERTY_ALLOW_VERIFYTABLEDEFINITION_EXPECTEDTABLE_COUNT_MISMATCH}, setting it to false.
 * <p>
...
public class DefaultExpectedDataSetAndVerifyTableDefinitionVerifier implements ExpectedDataSetAndVerifyTableDefinitionVerifier {
    ...
    protected void handleCountMismatch(final VerifyTableDefinition[] verifyTableDefinitions, final String[] expectedTableNames, final DatabaseConfig config) throws DataSetException {
        final String methodName = "handleCountMismatch";
        final int verifyTableDefsCount = verifyTableDefinitions.length;
        final int expectedTablesCount = expectedTableNames.length;
        final String msg = "{}: Test specified {} expected tables and {} VerifyTableDefinitions; usually these numbers should match as an expected table is not verified without a VerifyTableDefinition";
    ...
    protected Set<String> makeMismatchedTableNamesList(final VerifyTableDefinition[] verifyTableDefinitions, final String[] expectedTableNames) {
        final Set<String> tables = new HashSet<>();
        final String methodName = "makeMismatchedTableNamesList";
        for (final String expectedTableName : expectedTableNames) {
            final boolean isExpectedTableExist = isVerifyTableDefinitionsHasTable(verifyTableDefinitions, expectedTableName);
            if (!isExpectedTableExist) {
                final String msg = "{}: expected table name={} does not have a corresponding VerifyTableDefinition";
    ...
    protected boolean isVerifyTableDefinitionsHasTable(final VerifyTableDefinition[] verifyTableDefinitions, final String expectedTableName) {
        boolean isExpectedTableFound = false;
        for (int j = 0; j < verifyTableDefinitions.length && !isExpectedTableFound; j++) {
            final VerifyTableDefinition verifyTableDefinition = verifyTableDefinitions[j];
            final String definitionTableName = verifyTableDefinition.getTableName();
            isExpectedTableFound = expectedTableName.equals(definitionTableName);
        }
        return isExpectedTableFound;
    }

    protected void failOnMismatch(final DatabaseConfig config, final Set<String> mismatchCountTables) throws DataSetException {
        final String methodName = "failOnMismatch";
        final boolean allowCountMismatch = (Boolean) config.getProperty(DatabaseConfig.PROPERTY_ALLOW_VERIFYTABLEDEFINITION_EXPECTEDTABLE_COUNT_MISMATCH);
        final String willFailTestWord = allowCountMismatch ? " not" : "";
        log.info("{}: Property {} is set to {} so will{} fail test", methodName, DatabaseConfig.PROPERTY_ALLOW_VERIFYTABLEDEFINITION_EXPECTEDTABLE_COUNT_MISMATCH, allowCountMismatch, willFailTestWord);
        if (!allowCountMismatch) {
            final int mismatchCount = mismatchCountTables.size();
            final String msg = "The following " + mismatchCount + " expected tables do not have" + " corresponding VerifyTableDefinitions: " + mismatchCountTables + "\nSet property '" + DatabaseConfig.PROPERTY_ALLOW_VERIFYTABLEDEFINITION_EXPECTEDTABLE_COUNT_MISMATCH + "' to true to suppress test fail.";
