...
public class DbUnitAssertBase {
    private final FailureFactory junitFailureFactory = getJUnitFailureFactory();
    ...
    protected ComparisonColumn[] getComparisonColumns(final String expectedTableName, final Column[] expectedColumns, final Column[] actualColumns, final FailureHandler failureHandler) {
        final ComparisonColumn[] result = new ComparisonColumn[expectedColumns.length];
    ...
    protected boolean compareRowCounts(final ITable expectedTable, final ITable actualTable, final FailureHandler failureHandler, final String expectedTableName) throws Error {
        ...
        if (skipRowComparison) {
            ...
            if (expectedRowsCount != actualRowsCount) {
                ...
                final Error error = failureHandler.createFailure(msg, String.valueOf(expectedRowsCount), String.valueOf(actualRowsCount));
    ...
    protected void compareColumns(final Column[] expectedColumns, final Column[] actualColumns, final ITableMetaData expectedMetaData, final ITableMetaData actualMetaData, final FailureHandler failureHandler) throws DataSetException, Error {
        ...
        if (columnDiff.hasDifference()) {
            ...
            final Error error = failureHandler.createFailure(message, Columns.getColumnNamesAsString(expectedColumns), Columns.getColumnNamesAsString(actualColumns));
            log.error(error.toString());
            throw error;
        }
    }

    protected void compareTableCounts(final String[] expectedNames, final String[] actualNames, final FailureHandler failureHandler) throws Error {
    ...
    protected void compareTableNames(final String[] expectedNames, final String[] actualNames, final FailureHandler failureHandler) throws Error {
    ...
    public void assertWithValueComparer(final ITable expectedTable, final ITable actualTable, final FailureHandler failureHandler, final ValueComparator defaultValueComparator, final Map<String, ValueComparator> columnValueComparers) throws DatabaseUnitException {
        ...
        final boolean isTablesEmpty = compareRowCounts(expectedTable, actualTable, validFailureHandler, expectedTableName);
        if (isTablesEmpty) {
            return;
        }
    ...
    protected void failIfNecessary(final ITable expectedTable, final ITable actualTable, final FailureHandler failureHandler, final int rowNum, final String columnName, final Object expectedValue, final Object actualValue, final String failMessage) {
        if (failMessage != null) {
            final Difference diff = new Difference(expectedTable, actualTable, rowNum, columnName, expectedValue, actualValue, failMessage);
    ...
    protected Map<String, Map<String, ValueComparator>> determineValidTableColumnValueComparers(final Map<String, Map<String, ValueComparator>> tableColumnValueComparers) {
        ...
        if (tableColumnValueComparers == null) {
            ...
            log.debug("determineValidTableColumnValueComparers: using getDefaultTableColumnValueComparerMap()={} as tableColumnValueComparers={}", validMap, tableColumnValueComparers);
        } else {
    ...
    protected Map<String, ValueComparator> determineValidColumnValueComparers(final Map<String, ValueComparator> columnValueComparers, final String tableName) {
        ...
        if (columnValueComparers == null) {
            ...
            log.debug("determineValidColumnValueComparers: using getDefaultValueComparerMap()={} as columnValueComparers={} for tableName={}", validMap, columnValueComparers, tableName);
        } else {
